...
##### Конфигурация
В файле `gulpfile.js` находятся параметры, которые можно изменять, вот их описание:
Переменная `libs` cодержит пути к библиотекам, которые можно по необходимости добавлять с помощью Yarn.
...
Переменная `options` содержит настройки сборки.
```js
var options = {
    useTemplates: true,
    production: false,
